...
下的实验配置为唯一输入，产物按 `run_id` 落到 `research/results/`。
...
| `run_experiment.sh` | **仅服务器（GPU）** | 跑一个实验端到端：物化数据 → 逐 seed 注入 checkpoint/seed/trace 环境 → 模型采样 → samples + traces 入档（骨架，含 TODO 挂载点） |
| `sync_results.sh` | 服务器 | 同步产物：大文件（data/samples/traces/logs）入对象存储，小结果（manifest/metrics/plots）入 git |

## 用法

```bash
# 本地（纯 CPU）：物化数据 + 写 manifest
...
已提交**的 git 状态下运行，使 manifest 的 `git_sha` 可追溯、可回滚。
