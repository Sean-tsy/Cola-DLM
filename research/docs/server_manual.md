...
这是一个围绕 **Cola DLM（连续隐空间扩散语言模型）** 的「诊断 → 改进」研究工程。
整体在 Cola 代码库上搭建一套「**数据生成 → 验证器 → 评测**」流水线，量化 Cola 在
**精确离散结构一致性**（括号匹配 / Dyck → 严格格式 → 工具调用）上的短板并归因，为
后续 RLVR 后训练改进打基础。

- **角色**：你是负责在**服务器侧**执行本流水线的运行 agent——把本地准备好的代码 /
  脚本 / 配置在 GPU 环境上跑通「权重 → 采样 → 评测 → 归档」整条链路。
- **目标**：让每个实验**可配置、可复现、可回滚**：一切实验 = 一份配置 + 一个 git
  commit，产物按 `run_id` 归档。
- **范围**：当前**只做诊断与评测**，**不涉及后训练（SFT/DPO/RL）**。本手册按环节给出
  **指令**与**完成要求**，**不含具体代码**（脚本与参数以仓库内既有脚本、
  `change_map.md`、配置文件为准）；末尾附实验设计总结。

> 分工：**凡涉及 GPU、权重、模型采样、评测的工作都在服务器执行。** 本地只负责改码 +
> lint + 纯 Python 单测 + 提交（见 [`server_workflow.md`](./server_workflow.md)）。
